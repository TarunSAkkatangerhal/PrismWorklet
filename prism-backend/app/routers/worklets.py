...
def create_worklet(worklet_in: WorkletCreate, db: Session = Depends(get_db)):
    # Validate mentor
    mentor = db.query(User).filter(User.id == worklet_in.mentor_id, User.role == "Mentor").first()
    if not mentor:
...
def get_worklet_flexible(worklet_identifier: str, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "created_on": worklet.created_on.isoformat() if worklet.created_on else None,
        "year": worklet.year,
        "team": worklet.team,
...
def get_completed_worklets_for_mentor(mentor_email: str, db: Session = Depends(get_db)):
    ...
    # For now, return all completed worklets
    # TODO: Filter by mentor when relationship is properly set up
