...
export default function StudentChatPage() {
  ...
  const fetchConversations = async () => {
    try {
      console.log('Fetching worklet conversations...');
      const response = await secureAPI.get('/api/messages/conversations');
      console.log('Worklet conversations response:', response.data);
  ...
  const fetchRooms = fetchConversations; // Alias for compatibility
  ...
  const handleSelectRoom = (room) => {
    ...
    setSelectedUser(room);  // Keep for compatibility
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Send via REST API to worklet group
      await secureAPI.post('/api/messages/send', {
        receiver_id: 0,  // No specific receiver for group chat
        content: messageText,
