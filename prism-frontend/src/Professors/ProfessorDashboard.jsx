// Professor Dashboard - Shows worklets content with professor-focused UI
// Similar to student dashboard but for professor role
...
export default function ProfessorDashboard() {
  ...
  const [layout, setLayout] = useState(() => localStorage.getItem('professor_worklet_layout') || 'list');
  ...
  useEffect(() => {
    const savedLayout = localStorage.getItem('professor_worklet_layout');
    if (savedLayout) {
  ...
  useEffect(() => {
    localStorage.setItem('professor_worklet_layout', layout);
  }, [layout]);
  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      ...
      try {
        ...
        setUserName(sanitizeInput(me.name || me.email?.split('@')[0] || 'Professor', { maxLength: 50 }));
        ...
        <header className="flex justify-between items-center mb-[3vh]">
          <div>
            ...
            <p className="text-[clamp(0.875rem,1.2vw,1rem)] text-slate-500 dark:text-slate-400">
              Oversee and guide worklet progress
            </p>
          </div>
            ...
            <div className="flex items-start gap-[1.2vw]">
              {userProfileData?.avatar_url ? (
                ...
                <div
                  ...
                  style={{ backgroundColor: generateColorFromName(userProfileData?.name || 'Professor') }}
                >
                  <span>{getInitials(userProfileData?.name || 'Professor')}</span>
                </div>
              )}
                ...
                {processedWorklets.map((worklet) => (
                  ...
                  >
                    ...
                    <p className="text-[clamp(0.8rem,0.95vw,0.875rem)] text-slate-600 dark:text-slate-400 mb-[1vw] line-clamp-2">
                      {worklet.description || worklet.problem_statement || 'No description available'}
                    </p>
                    ...
                    <div className="flex items-center justify-between text-[clamp(0.7rem,0.85vw,0.75rem)] text-slate-500 dark:text-slate-400">
                      ...
                      {worklet.student_count !== undefined && (
                        <div className="flex items-center gap-1">
                          <Users className="w-3 h-3" />
                          <span>{worklet.student_count} {worklet.student_count === 1 ? 'Student' : 'Students'}</span>
                        </div>
                      )}
                    </div>
                  </div>
