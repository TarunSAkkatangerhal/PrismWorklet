      ...
      <div className={`max-w-[75%] ${isOwnMessage ? 'order-2' : 'order-1'}`}>
        ...
        >
          ...
          <div className={`flex items-center gap-1 justify-end mt-1`}>
            ...
            {isOwnMessage && (
              <svg className="w-4 h-4 text-gray-600 dark:text-gray-300" viewBox="0 0 16 15" fill="none">
...
export default function MentorChatPage() {
  ...
  const handleSelectRoom = (room, isGroup = false) => {
    ...
    fetchMessages(isGroup ? room.group_id : room.room_id, isGroup);
  };
  ...
  const handleViewGroupProfile = async () => {
    ...
    try {
      const profile = await chatService.getGroupProfile(selectedRoom.group_id);
